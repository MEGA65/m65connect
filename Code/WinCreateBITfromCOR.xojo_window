#tag Window
Begin Window WinCreateBITfromCOR
   Backdrop        =   0
   ...
   Begin TextField TextSourceFile
      ...
      TextAlignment   =   0
      TextColor       =   &c00000000
      Tooltip         =   "Source file of type COR"
      Top             =   54
      Transparent     =   False
      Underline       =   False
      ValidationMask  =   ""
	...
	#tag Event
		Sub Open()
		  ...
		  #If TargetWindows Or TargetMacOS Then
		    ...
		    PushSaveBIT.Height = 22
		    PushClose.Height = 22
		  #EndIf
		  
		  // Set window position
		  WinCreateBITfromCOR.Left = (MainWindow.Left + MainWindow.Width) - ((WinCreateBITfromCOR.Width + MainWindow.Width) /2)
		  WinCreateBITfromCOR.Top = MainWindow.Top + 40
		End Sub
	#tag EndEvent
...
#tag Events BevelAddSourceFile
	#tag Event
		Sub Action()
		  ...
		  FileType.Extensions = "cor;COR"
		  
		  Var Dialog As OpenDialog
		  Dialog = New OpenDialog
		  Dialog.Title = "Select COR file"
		  ...
		  Var FilePathCOR As FolderItem
		  FilePathCOR = Dialog.ShowModal
		  
		  If FilePathCOR <> Nil Then
		    TextSourceFile.Value = FilePathCOR.NativePath
		    
		    // Set SaveAs button
		    If Trim(TextSourceFile.Value).Length > 0 Then
		      PushSaveBIT.Enabled = True
		    Else
		      PushSaveBIT.Enabled = False
		    End If
		  End If
	...
	#tag EndEvent
#tag EndEvents
#tag Events PushSaveBIT
	#tag Event
		Sub Action()
		  // Get sourcefile (COR)
		  Var SourceFile As New FolderItem(TextSourceFile.Value)
		  
		  // Proceed if COR file still available
		  If SourceFile <> Nil Then
		    
		    // Set BIT file defintion
		    Var BITType As New FileType
		    BITType.Name = "binary/bit"
		    BITType.Extensions = "bit;BIT"
		    
		    Var TargetFileName As String
		    Var DialogBIT As New SaveFileDialog
		    
		    Var BITFile As FolderItem
		    DialogBIT.Filter = BITType
		    DialogBIT.SuggestedFileName = SourceFile.Name.Replace(".cor", ".bit")
		    DialogBIT.Title = "Save converted " + SourceFile.Name + " as..."
		    BITFile = DialogBIT.ShowModal
		    
		    If BITFile <> Nil Then
		      // Create the BIT file ouf of the core file by remove the header (4096 bytes)
		      Var SourceStream As BinaryStream = BinaryStream.Open(SourceFile, False)
		      Var TargetStream As BinaryStream = BinaryStream.Create(BITFile, True)
		      Var BITStruct As New MemoryBlock(SourceFile.Length -4096)
		      
		      SourceStream.BytePosition = 4096
		      BITStruct.StringValue(0, BITStruct.Size) = SourceStream.Read(SourceStream.Length-4095)
		      TargetStream.Write(BITStruct)
		      SourceStream.Close()
		      TargetStream.Close()
		      
		      MainWindow.StatusText.Value = "File "+BITFile.Name+" created"
		      WinCreateBITfromCOR.Close()
		    End If
